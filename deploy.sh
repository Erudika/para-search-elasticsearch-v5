...
sed -i -e "s/PARA_PLUGIN_VERSION=.*/PARA_PLUGIN_VERSION="\"$ver\""/g" Dockerfile && \
...
echo "" >> changelog.txt && \
echo "" >> changelog.txt && \
echo "### :package: [Download JAR](https://oss.sonatype.org/service/local/repositories/releases/content/com/erudika/para-search-elasticsearch-v5/${ver}/para-search-elasticsearch-v5-${ver}-shaded.jar)" >> changelog.txt && \
